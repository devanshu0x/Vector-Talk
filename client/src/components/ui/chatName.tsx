...
export function ChatName({ name, chatId }: ChatNameProps) {
    ...
    return <div>
        ...
        }} className="font-heading text-lg sm:text-xl md:text-2xl focus-visible::border-none focus-visible:ring-0 bg-background dark:bg-background border-none" value={chatName} onChange={(e) => setChatName(e.target.value)} />
        {
            isChanging && <Loader2 size={28} className="animate-spin opacity-70" />
        }
    </div>
    </div>
}